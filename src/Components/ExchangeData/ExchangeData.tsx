...
const ExchangeData = () => {
  ...
  const [selectedExchangeTickers, setSelectedExchangeTickers] =
    useState<ExchangeTickersResponse | null>(null);
  ...
  const getExchangeDataTickers = async () => {
    ...
    fetch(
      `https://api.coingecko.com/api/v3/exchanges/${exchangesId}/tickers`,
      options,
    )
      .then((res) => res.json())
      .then((res) => {
    ...
    <div className="exchange-data">
      <div className="mini-exchange">
        <div className="stages">
          <div className="stage">
            <p onClick={() => navigate("/cryptocurrencies")}>
              Cryptocurrencies
            </p>{" "}
            <MdKeyboardArrowRight />
          </div>
          <div className="stage">
            <p onClick={() => navigate("/crypto_exchanges")}> Exchanges </p>{" "}
            <MdKeyboardArrowRight />
          </div>
          <div className="stage">
        ...
        <div className="name-info">
          <div className="name">
            ...
            <p className="centralized">
              {selectedExchange?.centralized ? "Centralized Exchange" : ""}
            </p>
          </div>
        </div>
        <div className="row">
          <div className="col">
            ...
            <h3>
              {currency.symbol}
              {selectedExchange?.tickers?.[0]?.converted_volume?.usd.toLocaleString()}{" "}
              - BTC
            </h3>
          </div>
          <div className="col">
            <p>BTC 24h Trading volume</p>
            <h3>{selectedExchange?.trade_volume_24h_btc}</h3>
          </div>
          <div className="col">
            <p>Trust Score</p>
        ...
        <div className="spot-market">
          ...
          <div className="crypto-table">
            ...
            {selectedExchangeTickers?.tickers
              ?.slice(0, showMore)
              .map((item: any, index: number) => (
                <div className="table-layouts table_info" key={index}>
                  <p className="rank">{index + 1}</p>
                  <Link className="pic_name" to={`/coin/${item?.coin_id}`}>
                    <p className="name">{item.coin_id.toUpperCase()}</p>
                    <p>
                      {allCoin?.id === item.tickers?.coin_id &&
                      allCoin.image ? (
                        <img
                          src={allCoin.image}
                          alt={`Coin image for ${item.coin_id}`}
                        />
                      ) : (
                        <span></span>
                      )}
                  ...
                  <p className="spread_p">
                    {currency.symbol}
                    {item.last.toLocaleString()}
                  </p>
                  <p>{(item.bid_ask_spread_percentage || 0).toFixed(2)}%</p>
                  <p className="volume_p">
                    {item?.converted_volume?.usd.toLocaleString()}
                  </p>
            ...
            <div className="middle-info">
              <div className="sites">
                ...
                <div className="site">
                  <h3>Community</h3>
                  <div className="inner-site">
                    <a href={selectedExchange?.facebook_url}>
                      <FaFacebook />
                      Facebook
                    </a>
                    <a href={selectedExchange?.telegram_url}>
                      <FaTelegram />
                      Telegram
                    </a>
                    <a
                      className="none"
                      href={selectedExchange?.reddit_url}
                      target="_blank"
                    ...
                    <a
                      className="none"
                      href={selectedExchange?.other_url_1}
                      target="_blank"
                      rel="noopener noreferrer"
                    >
                      <FaLinkedin />
                ...
                <div className="site">
                  {selectedExchange?.address ? (
                    ...
                    <>
                      <h3>Coins</h3>
                      <div className="inner-site">
                        <p>{selectedExchange?.coins || "N/A"}</p>
                      </div>
                    </>
                  )}
                </div>
                ...
                <div className="site">
                  <h3>Rank</h3>
                  <div className="inner-site">
                    <p>{selectedExchange?.trust_score_rank || "-"}</p>
                  </div>
                </div>
                <hr />
                <div className="site">
